...
-- CASE 컬럼 
-- 	WHEN 비교값1 THEN 비교값1과 컬림이 일치할 때 결과
...
SELECT LENGTH(name) / 3 '사원이름 길이' FROM employee_view;

-- CONCAT(문자열, ...) : 매개변수로 전달받은 문자열들을 결합
-- 매개변수 중 하나라도 null이면 null을 반환
SELECT CONCAT(사원이름, 부서명) '사원과 부서명' FROM employee_view;

-- LEFT(문자열, 추출길이), RIGHT(문자열, 추출길이) : 문자열을 왼쪽 or 오른쪽에서부터 추출
SELECT LEFT(사원이름, 2) '왼쪽추출', RIGHT(사원이름, 2) '오른추출' FROM employee_view;

-- REPLACE(기준문자열의컬럼, 찾을문자열, 바꿀문자열) : 특정 문자열을 지정한 문자열로 변경하는 함수
SELECT REPLACE(사원이름, '길동', '순자') '문자열치환' FROM employee_view;
